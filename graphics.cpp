...
void Graphics::HierarchicalUpdate2(double dt) {
	...
	glm::vec3 planetPos = glm::vec3(modelStack.top()[3].x, modelStack.top()[3].y, modelStack.top()[3].z);
	...
	speed = { 6, 6, 6 };
	dist = { 1.25, 1.25, 0. };
	//rotVector = { dist }//{ 1.,0.,1. };
	rotSpeed = { .25, .25, .25 };
	scale = { .27f, .27f, .27f };

	modelStack.top() *= glm::translate(glm::mat4(1.f),
		glm::vec3(cos(speed[0] * dt) * dist[0], sin(speed[1] * dt) * dist[1], sin(speed[2] * dt) * dist[2]));
	//modelStack.top() *= glm::lookAt(modelStack.top()[1]., glm::vec3(0), glm::vec3(0, 1, 0));
	glm::vec3 shipPos = glm::vec3(modelStack.top()[3].x, modelStack.top()[3].y, modelStack.top()[3].z);
	glm::vec3 diff = shipPos - planetPos;
