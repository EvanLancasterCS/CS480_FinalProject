...
void Sphere::setupVertices() {
    ...
    for (int i = 0; i < numIndices; i++) {
        //Vertices.push_back(Vertex(vert[i], norm[0], tex[0]));
        Indices.push_back(i);
    }
...
void Sphere::init(int prec) {
    ...
    {
        Vertex* vert = new Vertex(glm::vec3(), glm::vec3(), glm::vec3());
    ...
    for (int i = 0; i <= prec; i++)
    {
        for (int j = 0; j <= prec; j++)
        {
            ...
            Vertices[i * (prec + 1) + j].vertex = glm::vec3(x, y, z);
            Vertices[i * (prec + 1) + j].normal = glm::vec3(x, y, z);
            Vertices[i * (prec + 1) + j].texcoord = glm::vec2(((float)j / prec), ((float)i / prec));
        }
    }
