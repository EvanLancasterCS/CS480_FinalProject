...
void Mesh::Update(int forward, int right)
{
	//if(forward > 0)
	...
	for (int i = 0; i < scene->mNumMeshes; i++) {
		...
		for (int j = 0; j < iMeshFaces; j++) {
			...
			for (int k = 0; k < 3; k++) {
				...
				Vertices.push_back(Vertex(glm::vec3(mesh->mVertices[face.mIndices[k]].x * scale, mesh->mVertices[face.mIndices[k]].y * scale, mesh->mVertices[face.mIndices[k]].z * scale),
